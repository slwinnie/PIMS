...
<strong>PIMS</strong> > <a href="~/Workbench/Default.aspx" class=" btn-default btn-create" runat="server">My Approval</a> > Risk Classification Form - Proposal <p></p>
    <h3 class="text-center">Risk Classification Form - Proposal</h3>
        <table class="table table-bordered table-hover">
            ...
            <tbody>
                <%
                    ...
                    var cnt = db.TApproval.Count(x => x.FormType == "Risk1" && x.EmployeeId == employeeId);
                    ...
                    var q = db.TApproval.Where(x => x.FormType == "Risk1" && x.EmployeeId == employeeId).OrderByDescending(x => x.CreatedDate).Skip(pageSize * (pageIndex - 1)).Take(pageSize);
                    var r = from x in q
                        join y in db.TProject.DefaultIfEmpty() on x.ProjectId equals y.ProjectId into tp
                        from y1 in tp
                        select new
                        {
                            x.ProjectId,
                    ...
                    foreach (var dr in r)
                    {
                        var item = db.RiskClassificationForm.FirstOrDefault(x => x.ProjectId == dr.ProjectId
                                                                                 && x.SubId == dr.SubId && x.type == "1");

                        sb.AppendFormat("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td></tr>",
                            dr.ProjectId,
                            string.Format("<a href='{4}Workbench/Forms/Risk1Form4Approval.aspx?projectId={0}&subId={1}&type=1'>{2}</a>&nbsp;<sup class='status'>{3}</sup>", dr.ProjectId, dr.SubId, dr.ProjectName, dr.Status, ResolveUrl("~")),
