...
<strong>PIMS</strong> > <a href="~/Workbench/Default.aspx" class=" btn-default btn-create" runat="server">My Approval</a> > New Project Information Sheet (NPIS) <p></p>

    <h3 class="text-center">New Project Information Sheet (NPIS) - Proposal</h3>
        <table class="table table-bordered table-hover">
            ...
            <tbody>
                <%
                    ...
                    var cnt = db.TApproval.Count(x => x.FormType == "Npis1" && x.EmployeeId == employeeId);
                    ...
                    var q = db.TApproval.Where(x => x.FormType == "Npis1" && x.EmployeeId == employeeId).OrderByDescending(x => x.CreatedDate).Skip(pageSize * (pageIndex - 1)).Take(pageSize);
                    var r = from x in q
                    ...
                    foreach (var dr in r)
                    {
                        var item = db.NPISForm.FirstOrDefault(x => x.ProjectId == dr.ProjectId
                                                         && x.SubId == dr.SubId && x.type == "1");
                        sb.AppendFormat("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td></tr>",
                            dr.ProjectId,
                            string.Format("<a href='{5}Workbench/Forms/NPISForm4Approval.aspx?projectId={0}&subId={1}&type=1'>{3}</a>&nbsp;<sup class='status'>{4}</sup>", dr.ProjectId, dr.SubId, dr.FormType, dr.ProjectName, dr.Status, this.ResolveUrl("~")),
