...
<strong>PIMS</strong> > <a href="~/Workbench/Default.aspx" class=" btn-default btn-create" runat="server">My Approval</a> > Risk Classification Form - Project<p></p>
    <h3 class="text-center">Risk Classification Form - Project</h3>
        <table class="table table-bordered table-hover">
            ...
            <tbody>
                <%
                    ...
                    var cnt = db.TApproval.Count(x => x.FormType == "Risk2" && x.EmployeeId == employeeId);
                    ...
                    var q = db.TApproval.Where(x => x.FormType == "Risk2" && x.EmployeeId == employeeId).OrderByDescending(x => x.CreatedDate).Skip(pageSize * (pageIndex - 1)).Take(pageSize);
                    var r = from x in q
                    ...
                    foreach (var dr in r)
                    {
                        ...
                        sb.AppendFormat("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td></tr>",
                            dr.ProjectId,
                            string.Format("<a href='{4}Workbench/Forms/Risk2Form4Approval.aspx?projectId={0}&subId={1}&type=2'>{2}</a>&nbsp;<sup class='status'>{3}</sup>", dr.ProjectId, dr.SubId, dr.ProjectName, dr.Status, ResolveUrl("~")),
