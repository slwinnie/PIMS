...
<%
    ...
    if (Request["async"] != null)
    {
        ...
        if (f != null)
        {
            ...
            var riskWorkshare = db.TRiskWorkshare.FirstOrDefault(x => x.ProjectId == iProjectId && x.SubId == iSubId && x.type == "2");
            var empId = Request["EmployeeId"];
            if (riskWorkshare != null)
            {
                riskWorkshare.t12 = SystemHelper.GetUserName(SystemHelper.DomainUserName());
                riskWorkshare.t13 = StringHelper.FmtDate(DateTime.Now);
                if (riskWorkshare.t12 == riskWorkshare.RejectBy)
                {
                    riskWorkshare.RejectBy = string.Empty;
                    riskWorkshare.RejectComment = string.Empty;
                }
            }
            #endregion
            ...
            var pj = db.TProjectX.FirstOrDefault(x => x.ProjectId == iProjectId && x.SubId == iSubId);
            if (pj != null)
            {
                pj.RiskApproveDate = DateTime.Now;
                if (pj.NPISSubmitDate != null 
                    && pj.NPISApproveDate != null)
                {
                    pj.Status = "Approved";
                }
            }
        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                var approvalInfo = db.TApproval.FirstOrDefault(x => x.ProjectId == iprojectId && x.SubId == iSubId
                    && x.EmployeeId.ToString() == emploeeId && x.FormType == "RiskWS");
...
<body>
    ...
    <div>
		...
		<div class="container">
			...
			<h2 class="text-center">WORKSHARE RISK PROMPT LIST<sup runat="server" id="status" style="color: red" class="pull-right"></sup></h2>
...
This checklist is based on the <strong style="text-decoration:underline;">Workshare Risk Management Guideline (CRP-0037)</strong> and is used to determine requirements for Support Office (SO) risk management. It shall be completed by the SO Proposal or Project Manager at bid, project award and subsequent new phases or significant project change. Please use in conjunction with <strong style="text-decoration:underline;">Workshare Risk Management Guideline (CRP-0037)</strong>
          ...
          <textarea id="t100" runat="server" class="table" cols="3" rows="3" style="background-color:#FF9" ></textarea>
    ...
    <script>
        $(function() {
            $("#approve").click(function () {
                $.ajax({
                    url: '<%=this.ResolveUrl("~")%>Workbench/Forms/RiskForm-WorkShare.aspx?async=1&projectId=' + $("#projectId").val() + "&subId=" + $("#subId").val() + "&employeeId=<%=SystemHelper.GetEmployeeId(SystemHelper.DomainUserName())%>&type=2&comments=" + encodeURIComponent($("#t100").val()),
                    type: "post",
                    datatype: "json",
                    success:function() {
                        //alert("success");
            ...
            $("#reject").click(function () {
                $.ajax({
                    url: '<%=this.ResolveUrl("~")%>Workbench/Actions/RiskWorkshareReject.aspx?projectId=' + $("#projectId").val() + "&subId=" + $("#subId").val() + "&employeeId=<%=SystemHelper.GetEmployeeId(SystemHelper.DomainUserName())%>&type=" + $("#type").val() + "&comments=" + encodeURIComponent($("#t100").val()),
                    type: "post",
                    datatype: "json",
                    success:function() {
