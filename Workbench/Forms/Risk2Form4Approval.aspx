    ...
    try
    {
        ...
        if (Request["async"] != null)
        {
            ...
            if (f != null)
            {
                ...
                GetRisk2FormNextAppover(Request["projectId"], Request["subId"], t);
                f.Status = "Approved";
                ...
                if (risk != null)
                {
                    ...
                    if (risk.t9 == empId)
                    {
                        risk.t11 = SystemHelper.GetUserName(SystemHelper.DomainUserName());
                        risk.t12 = StringHelper.FmtDate(DateTime.Now);
                        if (risk.t11 == risk.RejectBy)
                        {
...
<script runat="server">
    ...
    private void GetRisk2FormNextAppover(string pProjectId, string pSubId, string t)
    {
        var db = new PPPEntities();
        ...
        if (frm != null)
        {
            ...
            {
                // 批复完成
                var f = db.TProjectX.FirstOrDefault(x => x.ProjectId == iPprojectId && x.SubId == iPsubId);
                if (f != null)
                {
                    f.RiskApproveDate = DateTime.Now;
                    if (f.NPISApproveDate != null)
                    {
                        f.Status = "Approved";
                    }
                }
    ...
    <script>
        $(function() {
            $("#approve").click(function () {
                $.ajax({
                    url: '<%=this.ResolveUrl("~")%>Workbench/Forms/Risk2Form4Approval.aspx?async=1&projectId=' + $("#projectId").val() + "&subId=" + $("#subId").val() + "&employeeId=<%=SystemHelper.GetEmployeeId(SystemHelper.DomainUserName())%>&type=2&comments=" + encodeURIComponent($("#t100").val()),
                    type: "post",
                    datatype: "json",
                    success: function () {

                        window.location.href = "../RiskForm-Project.aspx";
                    },
            ...
            $("#reject").click(function () {
                $.ajax({
                    url: '<%=this.ResolveUrl("~")%>Workbench/Actions/RiskReject.aspx?projectId=' + $("#projectId").val() + "&subId=" + $("#subId").val() + "&type=2" + "&employeeId=<%=SystemHelper.GetEmployeeId(SystemHelper.DomainUserName())%>" + "&comments=" + encodeURIComponent($("#t100").val()),
                    type: "post",
                    datatype: "json",
                    success:function() {
                        //alert("success");
...
<strong>PIMS</strong> > <a href="/Workbench/Default.aspx" class=" btn-default btn-create">My Approval</a> > Risk Classification Form - Project<p></p>
    <div>
