...
<%
    ...
    if (db.NPISAttach.Any(x => x.ProjectId == iprojectId && x.SubId == isubId && x.type == type && x.FileName.ToLower() == file.FileName))
    {
    ...
    else
    {
        try
        {
            ...
            var svrFileName = fn;
            if (System.IO.File.Exists(Server.MapPath(string.Format("~/Forms/NPISFiles/{0}", svrFileName))))
            {
            ...
            if (db.NPISAttach.Any())
            {
                item.Seq = db.NPISAttach.Max(x => x.Seq) + 1;
            }
            else
            {
                item.Seq = 1;
            }
            db.NPISAttach.Add(item);
            db.SaveChanges();
            file.SaveAs(Server.MapPath(string.Format("~/Forms/NPISFiles/{0}", svrFileName)));
            var sb = new StringBuilder();
            sb.AppendFormat("{{");
